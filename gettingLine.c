 ...
 */
ssize_t my_bufchain(info_t *infoPtr, char **bufPtr, size_t *lenPtr)
{
	...
	if (!*lenPtr) /* fixing trailing white space */
	{
		...
		*bufPtr = NULL;
		...
		if (rSize > 0)
		{
			...
			infoPtr->linecount_flag = 1;
			my_rmcomment(*bufPtr);
			my_histori_make(infoPtr, *bufPtr, infoPtr->histcount++);
			{
				*lenPtr = rSize;
				infoPtr->cmd_buf = bufPtr;
			}
		}
	}
	return (rSize);
 ...
 */
ssize_t my_input(info_t *infoPtr)
{
	static char *buf;
	static size_t i, j, len;
	ssize_t rSize = 0;
	char **buf_pChar = &(infoPtr->arg), *pChar;
 ...
 * Return: r.
 */
ssize_t my_bufread(info_t *infoPtr, char *bufPtr, size_t *iPtr)
{
	ssize_t rSize = 0;

	if (*iPtr)
		return (0);
	rSize = read(infoPtr->readfd, bufPtr, READ_BUF_SIZE);
	if (rSize >= 0)
		*iPtr = rSize;
	return (rSize);
}
 ...
 * Return: the final s.
 */
int my_lineget(info_t *infoPtr, char **ptrChar, size_t *lengthPtr)
{
