...
int my_strtoint(char *sPtr)
{
	...
	if (*sPtr == '+')
		sPtr++;  /* TODO: why does main returned */
	for (x = 0;  sPtr[x] != '\0'; x++)
	{
 ...
 *        error on -1.
 */
void my_erroprint(info_pass *infoPtr, char *estrPtr)
{
	my_printstr(infoPtr->fnameChar);
	my_printstr(": ");
	my_decimprint(infoPtr->line_countInt, STDERR_FILENO);
	my_printstr(": ");
	my_printstr(infoPtr->argvChar[0]);
	my_printstr(": ");
	my_printstr(estrPtr);
}
